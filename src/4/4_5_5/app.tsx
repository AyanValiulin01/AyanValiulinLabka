...
export default function Page() {
  ...
  useEffect(() => {
    ...
    fetchData('/planets').then(result => {
      if (!ignore) {
        ...
        setPlanetId(result[0].id); // Выбираем первую планету
      }
    });
    return () => {
      ignore = true;
  ...
  useEffect(() => {
    ...
    fetchData(`/planets/${planetId}/places`).then(result => {
      if (!ignore) {
        console.log(`Fetched places for ${planetId}.`);
        setPlaceList(result);
        setPlaceId(result[0]?.id || ''); // Выбираем первое место, если есть
      }
    });
    return () => {
      ignore = true;
    };
  }, [planetId]); // Запускаем при изменении planetId
